...
async def test_zmq_comm():
    ...
    try:
        ...
        for i in range(3):
            ...
            comm = AsyncComm(logger, node_info=my_nodeinfo, parent_conn=parent_conn, hb_parent_conn=hb_parent_conn)
            comms.append(comm)
...
async def test_mp_comm():
    ...
    try:
        ...
        for i in range(nnodes):
            ...
            comm = AsyncComm(logger, node_info=my_nodeinfo, parent_conn=parent_conn, hb_parent_conn=hb_parent_conn, child_transport="mp")
            comms.append(comm)
...
async def test_comm_state_roundtrip_zmq():
    ...
    assert restored.parent_conn_state.remote_address == child_state.parent_conn_state.remote_address
    assert restored.data_transport_state.hostname == child_state.data_transport_state.hostname
