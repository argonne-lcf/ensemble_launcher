...
class MulPrivateActor(PrivateActor):
    @action
    def mul(self, a, b):
        return a * b
...
def test_actor_pool_construction():
    ...
    assert pool._actor_class_list == [AddPrivateActor] * 3
...
def test_actor_pool_has_actions():
    assert "invoke" in ActorPool.__actions__
    assert "invoke_all" in ActorPool.__actions__
    assert "invoke_all_stream" in ActorPool.__actions__
...
async def test_actor_pool_invoke():
    ...
    _, result = await asyncio.wait_for(
        handle.invoke(0, ("add", (3, 4), None)), timeout=30.0
    )
    assert result == 7

    _, result = await asyncio.wait_for(
        handle.invoke(1, ("add", (10, 20), None)), timeout=30.0
    )
    assert result == 30
...
async def test_actor_pool_invoke_all():
    ...
    # Same message to all (tuple)
    _, results = await asyncio.wait_for(
        handle.invoke_all(("add", (5, 6), None)),
        timeout=30.0,
    )
    assert len(results) == 3
    assert all(r == 11 for r in results)

    # Different message per actor (list of tuples)
    per_actor_msgs = [
        ("add", (1, 10), None),
        ("add", (2, 20), None),
        ("add", (3, 30), None),
    ...
    _, results = await asyncio.wait_for(
        handle.invoke_all(per_actor_msgs),
        timeout=30.0,
    )
    assert len(results) == 3
...
async def test_actor_pool_invoke_all_stream():
    ...
    streamed_results = []
    for _ in range(2):
        _, result = await asyncio.wait_for(handle.recv(), timeout=30.0)
...
async def test_actor_pool_get_n_actors_and_ids():
    ...
    _, n = await asyncio.wait_for(handle.get_n_actors(), timeout=30.0)
    assert n == 2

    _, ids = await asyncio.wait_for(handle.get_actor_ids(), timeout=30.0)
...
@pytest.mark.asyncio
async def test_actor_pool_mixed_classes():
    ckpt_dir = os.path.join("/tmp", f"ckpt_{uuid.uuid4()}")
    process = _start_cluster(ckpt_dir)

    transport = transport_registry.get("zmq")["transport"]()
    pool_secret = "pool_secret_mix"
    server, client = transport.create_child_pipe(
        "pool_parent_mix", pool_secret, "pool_mix", pool_secret
    )

    pool = ActorPool(
        name="pool_mix",
        client_conn=client,
        actor_class=[AddPrivateActor, MulPrivateActor],
        n_actors=2,
        actor_kwargs=[{}, {}],
        task_kwargs={"nnodes": 1, "ppn": 1},
        checkpoint_dir=ckpt_dir,
    )
    pool_task = pool.create_task(task_id="pool-mix-task", nnodes=1, ppn=1)
    ...
    handle = ActorPool.create_handle(
        server, default_target_id=f"pool_mix:{pool_secret}"
    )
    await handle.open()
    await handle.wait_for_ready(expected=1)

    # invoke add on child 0
    _, result = await asyncio.wait_for(
        handle.invoke(0, ("add", (3, 4), None)), timeout=30.0
    )
    assert result == 7

    # invoke mul on child 1
    _, result = await asyncio.wait_for(
        handle.invoke(1, ("mul", (3, 4), None)), timeout=30.0
    )
    assert result == 12

    # invoke_all with different actions per child
    _, results = await asyncio.wait_for(
        handle.invoke_all([
            ("add", (10, 20), None),
            ("mul", (10, 20), None),
