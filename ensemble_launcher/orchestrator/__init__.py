...
def load_external_policies():
    ...
    if policy_module:
        try:
            ...
            all_policies = (list(policy_registry.available_policies.keys()) + 
                          list(policy_registry.available_worker_policies.keys()))
            logger.info(f"External policies loaded. Available policies: {all_policies}")
            
        except ImportError as e:
            logger.warning(f"Could not import external policy module '{policy_module}': {e}")
        except Exception as e:
            logger.error(f"Error loading external policy module '{policy_module}': {e}", exc_info=True)
    elif policy_path:
        logger.info(f"EL_EXTERNAL_POLICY_PATH set but EL_EXTERNAL_POLICY_MODULE not specified. "
